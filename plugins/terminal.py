...
async def terminal_command(_: Client, msg: types.Message):
    ...
    except IndexError:await warn(msg, _, 'Enter an command!');return None
    else:
        cmd = ' '.join(str(msg.text).split(' ')[1:])
        await msg.edit('⌨️ <b>Command: </b><code>'+cmd+'</code><b>...</b>')
        await msg.edit('⌨️ <b>Command:</code> <code>'+cmd+'</code>\n<b>Output: </b>'+os.popen(cmd).read())
